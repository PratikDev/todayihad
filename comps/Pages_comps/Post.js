...
export default function Post({ data, count, separate }) {
  ...
  const isLoading = authContext === undefined;
  ...
  async function handleCookieClick() {
    const { sendCookie } = await import(`../../firebase/firebase_functions`);

    const sendCookieResult = await sendCookie({ postID, cookieSender: uid });

    console.log(sendCookieResult);
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="w-25" onClick={handleCookieClick}>
            <button
